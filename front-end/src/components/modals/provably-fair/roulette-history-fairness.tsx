...
import { RouletteFairnessHistoryData, useRouletteProvablyFairHistory } from '../../../queries/roulette';
import { findColorResult } from '../../lobby/roulette/utils';
import { notify } from '../../toast';

export const RouletteFairnessHistoryModal = ({ lobbyId }: ModalProps<'roulette-fairness-history-modal'>) => {
  ...
  const { data: provablyFairHistoryData } = useRouletteProvablyFairHistory(lobbyId);
    ...
    <div className="flex h-[560px] min-w-[650px] flex-col rounded-xl bg-[#152947] pr-1 max-md:w-screen max-md:min-w-0 max-md:rounded-none">
      ...
      <div className="relative flex items-center justify-center border-b border-b-[#12223B] px-9 py-[21px] max-md:px-3.5 max-md:py-4">
        <div className="flex gap-6 text-base font-extrabold">
          <span className="text-xl text-white">Roulette Game History</span>
        </div>
        <button
          onClick={() => closeModal()}
      ...
      <div className="scrollbar-thumb-rounded-full scrollbar-track-rounded-full scrollbar-thin scrollbar-thumb-[#253C60CC] scrollbar-track-[#6E88AF] h-[460px] w-full overflow-y-auto border-b border-[#12223B] px-6 py-6 text-lg font-extrabold text-[#6E88AF]">
        {provablyFairHistoryData && (
          <div className="flex w-full flex-col gap-3">
            {provablyFairHistoryData.map((gameData: RouletteFairnessHistoryData) => {
                      ...
                      <div className="flex max-w-[600px] flex-col gap-2 px-4 py-3 text-sm text-white">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-3 text-white">
                            ...
                            {sliceText(gameData.fairnessRandom, 15)}
                          </div>
                        </div>
                        <div className="flex items-center justify-between">
                          ...
                          <div
                            className={classNames(
                              ...
                              {
                                'bg-[#328C3C]': findColorResult(gameData.result.toString()) === 'green',
                                'bg-[#FF3C48]': findColorResult(gameData.result.toString()) === 'red',
                                'bg-[#364E71]': findColorResult(gameData.result.toString()) === 'black',
                              },
                            )}
                          >
                            {gameData.result}
